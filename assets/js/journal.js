...
async function loadJournal() {
  ...
  try {
    ...
    gallery.innerHTML = media.length ? media.map((item, index) => {
      if (item.type === 'embed') return `<figure class="photo-item video-item embed-item embed-preview"><iframe src="${escapeHtml(item.url)}" title="${escapeHtml(item.alt)}" loading="lazy" tabindex="-1"></iframe><button class="video-modal-trigger" type="button" data-video-url="${escapeHtml(item.url)}" data-video-title="${escapeHtml(item.alt)}" data-video-caption="${escapeHtml(item.caption || 'Video kỷ niệm')}" aria-label="Mở video ${escapeHtml(item.caption || 'kỷ niệm')}"><span>▶</span><b>Xem video</b></button>${item.caption ? `<figcaption>${escapeHtml(item.caption)}</figcaption>` : ''}</figure>`;
